// 🏁 달리기 대회 OBS 위젯용 공개 데이터 API (토큰 기반, 읽기 전용).
//   - /event-race/live 페이지가 이 API를 폴링해서 참가자·확정된 당첨자(1~K등) 명단을 받아 연출한다.
//   - 서바이벌 overlay API와 동일 패턴: 고정 토큰으로 overlay_token 접두 매칭 → 가장 최근 이벤트 1건.
//     달리기 이벤트는 overlay_token이 "race_luludongi_live_..." 로 저장된다(create 시 eventKind:"race").
//   - survivor_nicknames(당첨자 K명)를 그대로 "먼저 결승선 통과한 순서"로 위젯이 재현한다.
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!token) return json({ ok: false, message: "달리기 위젯주소 token이 없습니다." }, 400);
    ...
    if (error) {
      return json({
        ok: false,
        message:
          (error.message || "달리기 overlay 조회 실패") +
          " — survivor_nicknames 컬럼이 없다면 supabase/sql/event_survival_columns.sql 을 먼저 실행하세요.",
      }, 500);
    }
    if (!data) return json({ ok: false, message: "표시할 달리기 이벤트가 없습니다." }, 404);
