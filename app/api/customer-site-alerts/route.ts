...
async function readBox(sb: ReturnType<typeof getSupabaseAdmin>, sessionKey: string, phone: string) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 읽기
    return String(body.mode) === "box"
      ? await readBox(sb, sessionKey, phone)
      : await readAlert(sb, sessionKey, phone);
  } catch (e: any) {
    return NextResponse.json({ ok: false, error: String(e?.message ?? e) }, { status: 500 });
  }
}
