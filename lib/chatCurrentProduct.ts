...
export type CurrentProduct = { productId: string; productName: string; detailName: string; setAt: string } | null;
...
export async function getCurrentProductAt(sb: SupabaseClient, atIso: string): Promise<CurrentProduct> {
  try {
    const { data } = await sb
      ...
      .select("product_id,product_name,detail_name,cleared,set_at")
      .lte("set_at", atIso)
...
export async function setCurrentProduct(
  sb: SupabaseClient,
  opts: { productId: string; productName?: string; detailName?: string }
): Promise<void> {
  await sb.from("chat_current_product").insert({
    product_id: String(opts.productId).trim(),
    product_name: String(opts.productName || "").trim(),
    detail_name: String(opts.detailName || "").trim() || null,
    cleared: false,
  });
}

export async function clearCurrentProduct(sb: SupabaseClient): Promise<void> {
  await sb.from("chat_current_product").insert({ product_id: null, product_name: null, detail_name: null, cleared: true });
}
