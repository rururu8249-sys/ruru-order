...
function parseChatOrderCore(
  ...
  // [2026-08-16 실방송] "베이지55 안담아져요ㅜㅜ" — 안 된다는 불평은 주문이 아니다. 접수하면 재고까지 잠근다.
  if (/(안담|못담|안돼|안되|안뜨|안나와|안보여|안열려|오류|에러|먹통|품절|매진|없어요|없네)/.test(text)) {
    return { ...base, reason: "안 된다는 불평 (주문 아님)" };
  }
