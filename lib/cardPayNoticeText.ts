...
//
// [2026-09-20 2차] 사장님 「내용이 좀 부실한 느낌인데」 — 1차에서 너무 깎았다.
//   전하려는 뜻은 «카톡으로 카드결제 링크를 보냈으니 확인해달라» 두 토막인데
//   1차 문구는 「보냈다」와 「카드」가 빠져 통보처럼 읽혔다.
//   → 둘 다 되살리되 한 줄 한도 안에서 되살린다. 실측:
//       닉 2자  742px / 34px / 한 줄
//       닉 8자  853px / 31px / 한 줄   (아래 안내줄 905px·29px 과 비슷한 길이)
//       닉 10자 985px / 26px / 한 줄
//       닉 12자        → 2줄 (드문 경우)
//   「보냈어요 확인해주세요」처럼 두 동사를 다 쓰면 닉 9자부터 2줄이라 안 쓴다.
//   「카톡 카드결제 링크」만으로 «카톡에 와 있다»가 전달되고, 동사는 «확인해주세요» 하나면 된다.
...
export function buildCardPayNoticeText(nickname: string): string {
  ...
  return `💳 ${nick}님 카톡 카드결제 링크 확인해주세요 🙏`;
}
