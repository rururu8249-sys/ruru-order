...
const AUTO_THUMB_RULES: Array<[RegExp, string]> = [
  ...
  [/라면|컵라면/, "ramen"],
  [/음료|생수|드링크|주스|커피|차(?![가-힣])|티백|녹차|홍차|에이드|탄산수|탄산음료/, "drink"],
  [/과자|스낵|쿠키|간식|초콜릿|육포|대게|먹태|건어물|빵(?![가-힣])/, "snack"],
  [/세제|세탁|섬유유연|퐁퐁|샴푸|바디워시/, "detergent"],
  [/휴지|티슈|생활용품|주방용품|욕실|칫솔|치약|밥그릇|반려동물|반려용품|애견/, "daily"],
  // 가방(백팩) 먼저 — "백팩"의 "팩"이 스킨케어(팩)로 오인되지 않게
  [/백팩|배낭/, "backpack"],
  // 뷰티
  [/립스틱|립밤|립오일|립글로스|틴트|립(?![가-힣])/, "cosmetics"],
  [/향수|퍼퓸|오드|바디미스트|edp|edt/, "perfume"],
  [/마스크팩|스킨케어|스킨(?![가-힣])|로션|크림|세럼|앰플|토너|클렌징|선크림|패치|워시|클렌저|기미|필링|부스터|스파클링|더유핏|팩(?![가-힣])/, "skincare"],
