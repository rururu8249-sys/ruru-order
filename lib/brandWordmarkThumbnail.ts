...
// ── [2026-09-03 사장님 확정] 사진 없는 상품용 자동 썸네일 (v6 — 사장님 생성 일러스트 47종 + 무지 운동화 = 48종) ──
...
const AUTO_THUMB_RULES: Array<[RegExp, string]> = [
  // 특수
  [/랜덤|럭키박스|복불복|무료나눔|무나(?![가-힣])|당첨/, "randombox"],
  [/인형|장난감|피규어|토이(?![가-힣])/, "toy"],
  // 식품·생활 (— "티백"이 가방(백)으로 오인되지 않게 가방보다 먼저)
  [/라면|컵라면/, "ramen"],
  [/음료|생수|드링크|주스|커피|차(?![가-힣])|티백|녹차|홍차|에이드|탄산수|탄산음료|콤부차|티나는 스물넷/, "drink"],
  [/과자|스낵|쿠키|간식|초콜릿|육포|대게|먹태|건어물|쥐포|꾸이맨|대왕발|오징어|안주(?![가-힣])|올리브오일|식용유|빵(?![가-힣])/, "snack"],
  [/세제|세탁|섬유유연|퐁퐁|샴푸|바디워시|곰팡이|이염방지|건조기/, "detergent"],
  [/휴지|티슈|생활용품|주방용품|욕실|칫솔|치약|밥그릇|반려동물|반려용품|애견|봉투|분리수거|수세미|텀블러/, "daily"],
  // 가방(백팩) 먼저 — "백팩"의 "팩"이 스킨케어(팩)로 오인되지 않게
  [/백팩|배낭/, "backpack"],
  // 뷰티
  [/립스틱|립밤|립오일|립글로스|틴트|립(?![가-힣])/, "cosmetics"],
  [/향수|퍼퓸|오드|바디미스트|플로럴|머스크(?![가-힣])|시트러스|edp|edt/, "perfume"],
  [/마스크팩|스킨케어|스킨(?![가-힣])|로션|크림|세럼|앰플|토너|클렌징|선크림|패치|워시|클렌저|기미|필링|부스터|스파클링|더유핏|미스트|비누|필오프|마스크(?![가-힣])|청결제|쫀득폼|팩(?![가-힣])/, "skincare"],
  [/화장품|쿠션|팩트|섀도|마스카라|메이크업|블러셔|아이라이너|코스메/, "cosmetics"],
  [/영양제|비타민|유산균|글루타치온|콜라겐|효소(?![가-힣])|홍삼|오메가3|루테인|가르시아|카무트|브로멜라인|캡슐|맥스슬림/, "supplement"],
  [/마사지|안마|괄사/, "daily"],
  ...
  [/몽클레어|몽클레르|몽클(?![가-힣])/, "padding"],
  [/버버리|바바리/, "trench"],
  [/아우터|외투/, "trench"],
  [/상의|하의|의류|여성복|옷(?![가-힣])/, "tshirt"],
  // 신발 — 종류 + 브랜드별 실물 일러스트 (사장님 신발 시트)
  [/힐(?![가-힣])|하이힐|펌프스|로퍼|플랫슈즈|단화|구두/, "heels"],
  [/샌들|버켄/, "sandals"],
  [/크록스|슬리퍼|쪼리|뮬(?![가-힣])|아쿠아슈즈/, "crocs"],
  [/부츠|워커|어그|앵클/, "boots"],
  [/노다|norda/, "norda"],
  [/나이키|nike|조던|에어포스|에어맥스/, "nike"],
  [/호카|hoka/, "hoka"],
  [/아식스/, "asics"],
  [/오니츠카|타이거/, "onitsuka"],
  [/데상트/, "descente"],
  ...
  [/반스(?![가-힣])/, "vans"],
  [/푸마/, "puma"],
  [/뉴발란스|뉴발(?![가-힣])/, "newbalance"],
  [/운동화|스니커|러닝화|조깅화|신발|슈즈|미즈노|브룩스|살로몬|리복|온러닝/, "sneakers"],
  // 가방
  [/지갑|카드지갑/, "wallet"],
  ...
  [/목걸이/, "necklace"],
  [/귀걸이/, "earrings"],
  [/헤어핀|곱창밴드|스크런치|머리끈|헤어(?![가-힣])|헤어악세|헤어밴드/, "hairacc"],
  ...
  [/롱샴|롱샹/, "bag"],
  [/이솝|딥디크|딥티크|조말론|바이레도|마르지엘라|산타마리아|소바쥬|어딕트|어디틱/, "perfume"],
];
