-- ============================================================
-- [핫픽스 2026-08-11] 주문 제출 RPC: 재고 부족 시 주문 거부 추가
-- 기준: 2026-08-11 02:30 라이브 DB에서 추출한 현행 함수 원문(무변경)
--       + 재고차감 블록에 부족 거부(RAISE EXCEPTION) 단 한 곳 추가
-- 중복주문 방지·포인트·멱등성 로직 등 나머지는 100% 동일
-- 원복: rollback_20260811_stock_reject.sql 실행
-- ============================================================
  ...
  LOOP
    ...
    LOOP
      ...
      IF (
        ...
        -- [2026-08-11 핫픽스] 재고 부족 시 주문 거부 (기존: 조용히 0으로 깎고 무한 접수 → 오버셀)
        -- 예외 발생 → 이 함수의 주문 insert·포인트 차감 전체가 롤백되어 주문 자체가 접수되지 않음
        IF v_current_stock < v_deduct_qty THEN
          RAISE EXCEPTION '재고가 부족합니다. (상품ID: %, 색상: %, 사이즈: %, 남은재고: %개, 주문수량: %개)',
            v_inv_row.product_id,
            coalesce(nullif(trim(coalesce(v_inv_row.color, '')), ''), '없음'),
            coalesce(nullif(trim(coalesce(v_inv_row.size, '')), ''), '없음'),
            v_current_stock, v_deduct_qty;
        END IF;
