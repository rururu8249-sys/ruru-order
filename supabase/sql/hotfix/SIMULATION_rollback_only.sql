...
-- ============== 시뮬레이션 (product 55 알로밴딩바지: 블랙/M 재고 2 실데이터) ==============
create temp table sim_results(seq serial, step text, expect text, got text, pass boolean);

do $sim$
declare
  r jsonb; r2 jsonb; err text;
  g1 text := gen_random_uuid()::text;
  v_cnt integer; v_stock integer;
  rows1 jsonb;
begin
  -- T1: AAA가 블랙/M 2개 담기 → 성공해야 함 (재고 2, 타인선점 0)
  r := public.claim_cart_hold('sim-session-AAAAAA', '01000000001', '시뮬A', '테스트A',
        '[{"productId":"55","color":"블랙","size":"M","qty":2}]'::jsonb, 15);
  insert into sim_results(step, expect, got, pass) values
    ('T1 담기성공(AAA 블랙M 2개)', 'ok=true', (r->'results'->0->>'ok'),
     (r->'results'->0->>'ok')='true');

  -- T2: BBB가 블랙/M 1개 담기 → 거부 (AAA가 2개 선점, 가용 0)
  r := public.claim_cart_hold('sim-session-BBBBBB', '01000000002', '시뮬B', '테스트B',
        '[{"productId":"55","color":"블랙","size":"M","qty":1}]'::jsonb, 15);
  insert into sim_results(step, expect, got, pass) values
    ('T2 선착순거부(BBB 블랙M)', 'ok=false, available=0',
     format('ok=%s, available=%s', r->'results'->0->>'ok', r->'results'->0->>'available'),
     (r->'results'->0->>'ok')='false' and (r->'results'->0->>'available')='0');

  -- T3: BBB가 블랙/S 담기 → 거부 (재고 0)
  r := public.claim_cart_hold('sim-session-BBBBBB', '01000000002', '시뮬B', '테스트B',
        '[{"productId":"55","color":"블랙","size":"S","qty":1}]'::jsonb, 15);
  insert into sim_results(step, expect, got, pass) values
    ('T3 품절옵션거부(블랙S 재고0)', 'ok=false',(r->'results'->0->>'ok'),
     (r->'results'->0->>'ok')='false');

  -- T4: AAA가 수량 2→1로 줄여 재담기 → 성공 (본인 선점 교체)
  r := public.claim_cart_hold('sim-session-AAAAAA', '01000000001', '시뮬A', '테스트A',
        '[{"productId":"55","color":"블랙","size":"M","qty":1}]'::jsonb, 15);
  insert into sim_results(step, expect, got, pass) values
    ('T4 본인수량조정(AAA 2→1)', 'ok=true', (r->'results'->0->>'ok'),
     (r->'results'->0->>'ok')='true');

  -- T5: 풀린 1개를 BBB가 담기 → 성공 (즉시 복귀 확인)
  r := public.claim_cart_hold('sim-session-BBBBBB', '01000000002', '시뮬B', '테스트B',
        '[{"productId":"55","color":"블랙","size":"M","qty":1}]'::jsonb, 15);
  insert into sim_results(step, expect, got, pass) values
    ('T5 해제분재담기(BBB 1개)', 'ok=true', (r->'results'->0->>'ok'),
     (r->'results'->0->>'ok')='true');

  -- T6: 제3자 CCC(선점 없음)가 블랙/M 1개 바로 제출 → 거부되어야 함 (재고2 = AAA1+BBB1 전부 선점)
  ...
  begin
    ...
    insert into sim_results(step, expect, got, pass) values
      ('T6 제출차단(남의선점 보호)', '재고부족 예외', '통과됨(문제!)', false);
  exception when others then
    err := SQLERRM;
    insert into sim_results(step, expect, got, pass) values
      ('T6 제출차단(남의선점 보호)', '재고부족 예외', left(err, 60), err like '%재고가 부족합니다%');
  end;

  -- T7: BBB(선점 1개 보유)가 세션키로 제출 → 성공 + 본인 선점 자동해제 + 재고 2→1
  begin
    ...
    insert into sim_results(step, expect, got, pass) values
      ('T7 정상제출(BBB 본인선점)', 'ok + 선점해제 + 재고2→1',
       format('ok=%s, 남은선점=%s, 재고=%s', r2->>'ok', v_cnt, v_stock),
       (r2->>'ok')='true' and v_cnt = 0 and v_stock = 1);
  exception when others then
    insert into sim_results(step, expect, got, pass) values
      ('T7 정상제출(BBB 본인선점)', '성공', left(SQLERRM, 60), false);
  end;

  -- T8: AAA 선점 만료 처리 → CCC 담기 성공 (만료 즉시 복귀)
  ...
  r := public.claim_cart_hold('sim-session-CCCCCC', '01000000003', '시뮬C', '테스트C',
        '[{"productId":"55","color":"블랙","size":"M","qty":1}]'::jsonb, 15);
  insert into sim_results(step, expect, got, pass) values
    ('T8 만료즉시복귀(CCC 담기)', 'ok=true (만료선점 제외)', (r->'results'->0->>'ok'),
     (r->'results'->0->>'ok')='true');

  -- T9: 같은 주문 재제출(같은 group_id) → 중복방지 로직 유지 확인
  begin
    r2 := public.submit_customer_order_with_points(rows1, 0, '01000000002', '시뮬B', '테스트B', 'sim-session-BBBBBB');
    insert into sim_results(step, expect, got, pass) values
      ('T9 중복제출방지 유지', 'duplicate=true', format('duplicate=%s', r2->>'duplicate'),
       (r2->>'duplicate')='true');
  exception when others then
    insert into sim_results(step, expect, got, pass) values
      ('T9 중복제출방지 유지', 'duplicate=true', left(SQLERRM, 60), false);
  end;
end;
$sim$;


-- ============== T10/T11: 재고관리 켜진 "모든" 상품 자동 검증 ==============
create function pg_temp.try_jsonb(t text) returns jsonb language plpgsql as
$tj$ begin return t::jsonb; exception when others then return null; end; $tj$;

do $multi$
declare
  v_prow record; r jsonb;
...
begin
  for v_prow in
    select p.id::text as pid, p.product_name,
           v->>'color' as color, v->>'size' as size, (v->>'stock')::int as stock
    from public.products p
    cross join lateral jsonb_array_elements(coalesce(pg_temp.try_jsonb(p.product_note)->'stock_variants','[]'::jsonb)) as v
    where lower(coalesce(pg_temp.try_jsonb(p.product_note)->>'stock_management_enabled','')) in ('true','t','1','yes','y')
      and coalesce(v->>'stock','') ~ '^[0-9]+$' and (v->>'stock')::int > 0
    order by p.id desc, (v->>'stock')::int desc
    limit 8
  loop
    v_n := v_n + 1;
    -- A가 전량 담기 → 성공 기대
    r := public.claim_cart_hold('sim-multi-A-' || v_n, '01000000011', '멀티A', '멀티A',
          jsonb_build_array(jsonb_build_object('productId', v_prow.pid, 'color', v_prow.color, 'size', v_prow.size, 'qty', v_prow.stock)), 15);
    ok1 := (r->'results'->0->>'ok') = 'true';
    -- B가 1개 담기 → 거부 기대 (전량 선점됨)
    r := public.claim_cart_hold('sim-multi-B-' || v_n, '01000000012', '멀티B', '멀티B',
          jsonb_build_array(jsonb_build_object('productId', v_prow.pid, 'color', v_prow.color, 'size', v_prow.size, 'qty', 1)), 15);
    ok2 := (r->'results'->0->>'ok') = 'false';
    -- A 해제(빈 담기) 후 B 1개 → 성공 기대 (즉시 복귀)
    r := public.claim_cart_hold('sim-multi-A-' || v_n, '01000000011', '멀티A', '멀티A', '[]'::jsonb, 15);
    r := public.claim_cart_hold('sim-multi-B-' || v_n, '01000000012', '멀티B', '멀티B',
          jsonb_build_array(jsonb_build_object('productId', v_prow.pid, 'color', v_prow.color, 'size', v_prow.size, 'qty', 1)), 15);
    ok3 := (r->'results'->0->>'ok') = 'true';

    insert into sim_results(step, expect, got, pass) values
      (format('T10-%s [%s] %s/%s 재고%s', v_n, left(v_prow.product_name, 14),
              coalesce(nullif(v_prow.color,''),'-'), coalesce(nullif(v_prow.size,''),'-'), v_prow.stock),
       '전량담기ok/추가담기거부/해제후ok',
       format('담기=%s/거부=%s/복귀담기=%s', ok1, ok2, ok3),
       ok1 and ok2 and ok3);

    if v_n = 1 then v_pid1 := v_prow.pid; v_c1 := v_prow.color; v_s1 := v_prow.size; end if;
    if v_n = 2 then v_pid2 := v_prow.pid; v_c2 := v_prow.color; v_s2 := v_prow.size; end if;
  end loop;

  if v_n = 0 then
    insert into sim_results(step, expect, got, pass) values
      ('T10 다중상품', '재고관리 상품 존재', '재고>0 상품 없음(검증 불가)', false);
  end if;

  -- T11: 서로 다른 두 상품을 "한 번의 담기"로 → 둘 다 성공 (다중 잠금 순서 검증)
  if v_pid1 is not null and v_pid2 is not null then
    r := public.claim_cart_hold('sim-multi-C', '01000000013', '멀티C', '멀티C',
          jsonb_build_array(
...
select step, expect, got, case when pass then '✅' else '❌' end as 판정
from sim_results order by seq;
