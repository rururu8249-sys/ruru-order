...
begin
  ...
  values (
    ...
    v_order_ids,
    format('닉네임 %s · 옛번호 %s → 새번호 %s', coalesce(r.nickname, ''), v_old_phone, v_new_phone)
  );

  -- [2] 옛 줄에 카톡ID «먼저» (번호는 아직 그대로라 트리거는 안 돈다)
  update customers
     set kakao_id = trim(r.new_kakao_id)
   where id = v_old.id
     and coalesce(trim(kakao_id), '') = ''
     and coalesce(trim(r.new_kakao_id), '') <> '';
