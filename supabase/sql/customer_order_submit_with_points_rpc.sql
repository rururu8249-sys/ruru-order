-- ============================================================================
-- ⚠️⚠️⚠️ 실행 금지 / DO NOT RUN ⚠️⚠️⚠️  (2026-06-25 폐기 표시)
-- 이 파일은 submit_customer_order_with_points 를 "재고차감 없이" 정의합니다.
-- 라이브 DB의 현재 함수는 재고 자동차감 버전입니다(inventory_auto_deduct_rpc.sql 계열).
-- ▶ 이 파일을 Supabase에서 다시 실행하면 재고 자동차감이 조용히 사라집니다(오버셀 사고).
-- ▶ 주문 제출 RPC의 단일 기준 = supabase/sql/inventory_auto_deduct_rpc.sql 입니다.
--   (이 파일은 포인트 부분 기록용으로만 보존. 절대 단독 실행하지 말 것)
-- ※ 정식 단일화는 배포본 함수 본문(pg_get_functiondef) 확인 후 진행 예정.
-- ============================================================================
