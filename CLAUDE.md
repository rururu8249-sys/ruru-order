...
- 고객 › 회원 목록: 필터 한 줄 `[검색·고객상태·정렬·주문있는고객만·초기화(바뀌었을 때만)]`, 입력칸 포커스는 로즈 focus-visible(파란 테두리 금지). **PC(xl 이상)=고객이슈 탭과 같은 표**(칸 `☐·사진·닉네임/이름·최근주문·주문·누적금액·포인트·상태·전화번호·처리`, 숫자칸 오른쪽정렬, 줄 py-2.5) / **좁은 화면=카드 유지**. 사진=kakao_profile_image 있으면 썸네일·클릭 640px 확대(기본사진 URL은 사진없음). 상태=이슈N·매칭필요·알림OFF·카카오미연동(해당할 때만), 차단회원은 왼쪽 빨간 띠+상태칸 사유요약(hover 전체). **최근주문은 orderCount>0일 때만**(latestOrderAt은 0건이면 가입일 폴백이라 그대로 쓰면 안 됨 → 0건은 「-」). 카카오 사진 저장 컬럼은 kakao_profile_image 하나뿐(썸네일·기본여부 미저장). **포인트 잔액·미해결이슈는 RLS로 브라우저 anon이 못 읽음 → 서버 라우트 `/api/admin-live/customer-list-extra`(service_role)로 현재 20명만 batch 조회**(회원별 개별 쿼리 금지).
