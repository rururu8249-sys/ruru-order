...
## 남은 작업 (우선순위 순)
1. 화면 전체 용어·버튼·라벨·문구 시안 기준 전수 통일 (입금상태뿐 아니라 admin-live 전 화면 일괄, 일부만 고치지 말 것)
2. 사이드바 "상품" 메뉴 → 상품관리 팝업 (AdminLiveProductListPanel 재활용, 코드 남아있음. 현재 상품 등록·고정·수정 진입 경로 없음)
3. 카드미결제 배지 클릭 → 복사창(상품명/금액/닉네임/전화 각 복사 + 결제완료처리) + 페이스터 window.open 좌우분할 (iframe 불가, 복사창 안닫히게, PC만 자동배치)
