...
- 고객 › 회원 목록: 필터 한 줄 `[검색·고객상태·정렬·주문있는고객만·초기화(바뀌었을 때만)]`, 입력칸·닉네임 버튼 포커스는 로즈 focus-visible(파란 테두리 금지). **PC(xl 이상)=고객이슈 탭과 같은 표**, 머리글·줄이 «같은 grid 템플릿». 최종 칸 순서 `[☐·사진·닉네임/이름·전화번호·상태(1fr 가장 넓게)·최근주문·최근로그인·주문·누적결제·포인트·처리]`, 숫자칸 고정폭 오른쪽정렬 / **좁은 화면=카드 유지**. **누적 결제=결제완료(isPaid)만 합산**(paidAmount, 판정 로직 무변경·합산 대상만) — 목록 총합과 상세 누적이 일치. 정렬 「누적 결제순」도 paidAmount 기준. 상태=**미입금 N건**(unpaidCount=isUnpaid, manual_match_needed 포함 → 상세 「미입금」과 통일)·이슈N·🔕알림OFF(글자 배지·여러 개 줄바꿈)·카카오미연동, 차단회원은 상태칸에 사유요약+hover 전체. 최근주문·최근로그인은 **고객이슈 표와 같은 두 줄 날짜**(`2026.09.25(금)`/`06:48`, 주문은 orderCount>0일 때만·없으면 「-」). last_login_at은 기존 select에 이미 있음(새 쿼리 금지). **차단 빨간 띠는 border-left가 아니라 inset box-shadow**(칸 폭에 영향 없게). 사진=kakao_profile_image 있으면 썸네일·클릭 640px 확대(기본사진 URL은 사진없음, 저장 컬럼은 이거 하나뿐). **포인트 잔액·미해결이슈는 RLS로 브라우저 anon이 못 읽음 → 서버 라우트 `/api/admin-live/customer-list-extra`(service_role·middleware+verifyAdminSession 이중보호·전화 최대 50개)로 현재 페이지 20명만 batch 조회**(회원별 개별 쿼리 금지).
