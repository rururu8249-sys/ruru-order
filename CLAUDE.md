...
[admin-live 관리자]
1. 화면 전체 용어·버튼·라벨·문구 시안 기준 전수 통일 (입금상태뿐 아니라 전 화면 일괄, 일부만 고치지 말 것) — 입금상태·메뉴·주요 팝업제목 1차 완료, 정산/입금매칭(admin-v2 의존) 제목은 남음
2. 카드미결제 배지 클릭 → 복사창(상품명/금액/닉네임/전화 복사 + 결제완료처리) + 페이스터 window.open 좌우분할 (iframe 불가, 복사창 안닫히게, PC만 자동배치)
3. 이벤트 상세페이지
4. 수동매칭 설정 팝업 메뉴
