...
export default function AdminLiveCustomerBlockReasonModal({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[var(--color-ink-soft)]/35 px-4">
      <section className="w-full max-w-[520px] rounded-2xl border border-danger-tx bg-surface p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-[11px] font-black tracking-[0.18em] text-danger-tx">BLOCK REASON</div>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button
            ...
            className="h-11 rounded-xl bg-[var(--color-danger-tx)] px-5 text-sm font-black text-white hover:bg-[var(--color-danger-tx)] disabled:cursor-not-allowed disabled:bg-danger-bg"
          >
            {saving ? "저장중..." : "차단 저장"}
          </button>
        </div>
