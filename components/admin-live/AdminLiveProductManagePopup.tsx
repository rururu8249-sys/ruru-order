...
// [2026-08-13] 📢 채팅 문구 — 유튜브 채팅창에 붙여넣을 상품 안내글을 만든다(사장님이 복사→수동 붙여넣기).
//   유튜브 채팅은 1회 200자 제한 → 세부상품이 많으면 「브랜드 3개 외 N브랜드 총 N종」으로 요약.
//   읽기 전용(화면 표시·복사만) — DB·주문·재고 로직과 무관.
const CHAT_MSG_LIMIT = 200;
function buildChatAnnounceText(p: ProductRow): string {
  const note = parseProductNote(p);
