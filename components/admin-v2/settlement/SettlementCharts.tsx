...
export default function SettlementCharts({
  ...
  const maxTrend = Math.max(...trend.map((item) => Math.max(item.sales, item.net, item.fee)), 1);
  const paymentTotal = Math.max(stats.bankAmount + stats.cardAmount + stats.otherAmount, 1);

  const paymentRows = [
    ["무통장", stats.bankAmount, "bg-emerald-500"],
    ["카드", stats.cardAmount, "bg-blue-500"],
    ["기타", stats.otherAmount, "bg-slate-400"],
    ...
    <div className="grid gap-5 xl:grid-cols-[1.35fr_0.65fr]">
      <div className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-[0_18px_45px_rgba(15,23,42,0.05)]">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-lg font-black text-slate-950">기간별 매출·수수료 흐름</div>
        ...
        {trend.length === 0 ? (
          ...
          <div className="mt-6 flex h-[220px] items-end gap-2 rounded-3xl bg-slate-50 p-4">
            {trend.map((item) => {
              ...
              const feeHeight = Math.max(4, Math.round((toNumber(item.fee) / maxTrend) * 160));
              ...
              return (
                <div key={item.dateKey} className="flex min-w-0 flex-1 flex-col items-center justify-end gap-2">
                  <div className="flex h-[170px] items-end gap-1">
                    <div title={`매출 ${won(item.sales)}`} className="w-3 rounded-t-full bg-blue-500" style={{ height: salesHeight }} />
                    <div title={`실수익 ${won(item.net)}`} className="w-3 rounded-t-full bg-emerald-500" style={{ height: netHeight }} />
                    <div title={`수수료 ${won(item.fee)}`} className="w-3 rounded-t-full bg-rose-400" style={{ height: feeHeight }} />
                  </div>
      ...
      <div className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-[0_18px_45px_rgba(15,23,42,0.05)]">
        <div className="text-lg font-black text-slate-950">결제수단 비중</div>
        <div className="mt-1 text-xs font-bold text-slate-400">완료 매출 기준</div>

        <div className="mt-6 grid gap-4">
          {paymentRows.map(([label, amount, color]) => (
            <div key={label}>
              ...
              <div className="h-3 overflow-hidden rounded-full bg-slate-100">
                <div
                  ...
                  style={{ width: `${Math.max(3, Math.min(100, (toNumber(amount) / paymentTotal) * 100))}%` }}
                />
              </div>
            </div>
          ))}
