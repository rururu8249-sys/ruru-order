    ...
    <div className="overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.05)]">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-6 py-4">
        <div>
          ...
          <div className="mt-1 text-xs font-bold text-slate-400">현재 필터에 포함된 주문을 방송 날짜 기준으로 묶어 보여줍니다.</div>
        </div>
        ...
        <table className="min-w-[1080px] w-full border-separate border-spacing-0">
          <thead>
            <tr className="bg-slate-50 text-xs font-black text-slate-500">
              ...
              <th className="px-4 py-3 text-right">총 주문금액</th>
              <th className="px-4 py-3 text-right">입금/결제완료</th>
              ...
              <th className="px-4 py-3 text-right">카드추가금</th>
              <th className="px-4 py-3 text-right">미입금</th>
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row) => (
                <tr key={row.key} className="border-b border-slate-100 hover:bg-blue-50/30">
                  ...
                  <td className="border-b border-slate-100 px-4 py-4 text-right text-sm font-black tabular-nums text-slate-700">{won(row.customerCardExtra)}</td>
