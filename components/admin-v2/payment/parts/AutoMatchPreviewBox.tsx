  ...
  return (
    <section className="mb-3 rounded-xl border border-amber-200 bg-amber-50 p-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="text-[13px] font-black text-amber-950">자동후보 결과</div>
          <div className="mt-1 text-[11px] font-bold text-amber-800">
            닉네임+금액이 정확히 맞는 1:1 후보만 보여줍니다. 애매한 건은 수동매칭으로 남깁니다.
          </div>
        </div>

        <div className="flex flex-wrap gap-2">
          <div className="rounded-lg bg-white px-3 py-2 text-[12px] font-black text-amber-900">
            후보 {candidateCount.toLocaleString()}건
          </div>

          <button
            type="button"
            onClick={onRun}
            disabled={autoRunLoading || candidateCount <= 0}
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-6">
        ...
        <CountCard label="미입금 주문" value={`${Number(summary.eligible_unpaid_orders || 0).toLocaleString()}건`} />
      ...
      {candidates.length === 0 ? (
        <div className="mt-3 rounded-lg bg-white p-3 text-[12px] font-black text-neutral-600">
          자동확정 가능한 후보가 없습니다. 그대로 수동매칭으로 처리하면 됩니다.
        </div>
      ) : (
