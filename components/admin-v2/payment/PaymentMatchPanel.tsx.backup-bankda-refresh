...
export default function PaymentMatchPanel({ deposits, orderGroups, onOpenManualMatch }: Props) {
  ...
  const summary = useMemo(() => ({
    ...
    unconfirmedDeposits: deposits.filter((deposit) => !isDepositConfirmed(deposit)).length,
    confirmedDeposits: deposits.filter((deposit) => isDepositConfirmed(deposit)).length,
  }), [deposits, orderGroups]);
      ...
      <section className="rounded-xl border border-neutral-200 bg-white p-3">
        <div className="mb-3">
          ...
          <div className="mt-1 text-[12px] font-bold text-neutral-500">
            자동입금확인이 안 된 주문을 입금내역과 직접 연결합니다. 주문관리의 입금매칭 버튼과 같은 처리 흐름입니다.
          </div>
        </div>
