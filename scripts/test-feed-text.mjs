...
// [2026-09-20 사장님] 「긴 알림은 좌우 여백 살짝 띄우고… 폰트도 좀 키우고」 → 2줄 · 절반씩 균형(알약 77%)
// [2026-09-24 2차 사장님] 「좌우 여백 꽉차게(최소의 여백만 남기고) 글씨 한줄에 최대한 많이」
//   → 절반 균형을 폐기했다. 2줄이어도 첫 줄이 폭을 꽉 채운다(알약 77% → 99%).
//     글자 크기는 그대로 34px 이고, 첫 줄에 들어가는 글자 수만 늘어난다.
{
  ...
  assert.equal(L.maxWidth, FEED_PIN_AVAIL_W, "2줄이어도 글자 칸은 폭을 꽉 써야 한다(절반 균형 폐기)");
